...
var _beetle_buff_value: int = 0

func _ready():
	var deathrattle_node = Deathrattle.new()
	add_child(deathrattle_node)
	deathrattle_node.触发卡 = self
	deathrattle_node.script = preload("res://所有卡牌/随从/野兽/符文虫祖/符文虫祖_Deathrattle.gd")
...
func _trigger_avenge():
	var buff_increase: int = 2
	if is_gold:
		buff_increase = 4
	_beetle_buff_value += buff_increase
