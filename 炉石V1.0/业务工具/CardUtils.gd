...
static var ALL_CARD:Dictionary[String,CardEntity]={}

static func 游戏初始化加载卡牌(加载种族:Array[Enums.CardRace]):
	# 加载随从
...
static func get_card(nameInfo:String,player:Player)->CardEntity:
	var card=(ALL_CARD.get(nameInfo) as CardEntity).duplicate()
	card.player=player
	card.信号绑定()
	return card
...
static func find_card(conditionList:Array[CardFindCondition])->Array:
	return ALL_CARD.values().filter(func (card):
...
static func scan_scenes_recursive(base_path: String, current_path: String = "",scene_dict:Dictionary = {}):
	...
	while file_name != "":
		...
		elif file_name.begins_with("test_"):
			pass
